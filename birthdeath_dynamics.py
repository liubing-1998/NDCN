...
parser = argparse.ArgumentParser('BirthDeath Dynamics Case')
...
# 只有BirthDeathDynamics上随机初始化
parser.add_argument('--init_random', action='store_true', help='if True then random init x value of t0')
# parser.add_argument('--power_law_param', type=int, default=4, help='power_law parameter default=5, in BirthDeath is 4')
...
if args.viz:
    # dirname = r'figure/birthdeath/' + args.network
    dirname = r'figure/birthdeath/' + args.network + '/' + args.count
    makedirs(dirname)
    fig_title = r'birthdeath Dynamics'

if args.dump:
    # results_dir = r'results/birthdeath/' + args.network
    results_dir = r'results/birthdeath/' + args.network
...
if args.viz:
    ...
    path = r'figure/birthdeath/' + 'network' + args.count + '/'
    makedirs(path)
...
if __name__ == '__main__':
    # 保存日志信息
    makedirs(r'log/')
    log_filename = r'log/BirthDeath' + "_" + args.network + "_NDCN_" + args.count + ".txt"
...
# python BirthDeath_dynamics.py --network grid --count 0
