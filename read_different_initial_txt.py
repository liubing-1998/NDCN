...
def read_different_initial_txt(file_path, count, dynamics):
    with open(file_path, "r") as f:
        ...
        with open('different_initial_interpolation_extrapolation_longtime/different_initial_'+dynamics+'.csv', 'a', encoding='utf-8', newline='') as f:
            # 2. 基于文件对象构建 csv写入对象
...
def read_different_A_initial_txt(file_path, count, dynamics):
    with open(file_path, "r") as f:
        file = f.read()
        batch = re.split('============================================', file)
        result_list = []
        ...
        with open('different_A_initial_interpolation_extrapolation_longtime/different_A_initial_'+dynamics+'.csv', 'a', encoding='utf-8', newline='') as f:
            # 2. 基于文件对象构建 csv写入对象
            csv_writer = csv.writer(f)
            # 3. 构建列表头
            csv_writer.writerow(['name', 'TestLoss1', 'TestLoss1Relative', 'TestLoss2', 'TestLoss2Relative', 'TestLoss3', 'TestLoss3Relative'])
...
if __name__ == '__main__':
    for dynamics in ['biochemical', 'birthdeath', 'heat']:
        file_path = r'different_initial_interpolation_extrapolation_longtime/different_initial_'+dynamics+'.txt'
        read_different_initial_txt(file_path, 9, dynamics)

        file_path_A = r'different_A_initial_interpolation_extrapolation_longtime/different_A_initial_'+dynamics+'.txt'
        read_different_A_initial_txt(file_path_A, 10, dynamics)
