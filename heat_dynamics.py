...
parser = argparse.ArgumentParser('Heat Dynamics Case')
...
if args.viz:
    ...
    dirname = r'figure/heat/' + args.network + '/' + args.count
    makedirs(dirname)
...
if args.network == 'grid':
    ...
    print("Choose graph: " + args.network)
    G = nx.newman_watts_strogatz_graph(n, 5, 0.5, seed=seed)  # 400, 5, 0.5
    G = networkx_reorder_nodes(G, args.layout)
    A = torch.FloatTensor(nx.to_numpy_array(G))
...
if args.viz:
    ...
    path = r'figure/heat/' + 'network' + args.count + '/'
    makedirs(path)
...
if __name__ == '__main__':
    # 保存日志信息
    makedirs(r'log/')
    log_filename = r'log/' + args.baseline + '_heat_' + args.network + "_" + args.count + ".txt"
    logging.basicConfig(filename=log_filename, level=logging.DEBUG, format='%(asctime)s %(message)s',
                        datefmt='%Y/%m/%d %I:%M:%S %p')
    logging.getLogger("matplotlib").setLevel(logging.WARNING)
    logging.info(
        "*********************************" + args.count + '_' + appendix + "*********************************")
    # 记录下参数和seed值
    logging.info(args)
        ...
        if args.sampled_time == 'irregular':
            ...
            print('Iter {:04d}| Train Loss {:.6f}({:.6f} Relative) '
                  ...
                  .format(itr, loss_train.item(), relative_loss_train.item(),
                          ...
                          time.time() - t_start))
            logging.info(
                'RESULT {:04d}| Train Loss {:.6f}({:.6f} Relative) | Test Loss {:.6f}({:.6f} Relative) | Time {:.4f}'
                .format(itr, loss_train.item(), relative_loss_train.item(), loss.item(), relative_loss.item(),
                        time.time() - t_start))
