...
def dx_dt_figure_heat_result(filename, seed=876, init_random=True, T=60, time_tick=1200):
    ...
    # 绘制模型打印的导数图
    dx_dt_model_figure(output, dx_dt, T, time_tick)

    # 绘制真实的导数图
    dx_dt_true_figure(true_y, L, t, T, time_tick)


def dx_dt_model_figure(output, fx_dt, T, time_tick):
    ...
    for i in range(0, 100, 10):
        ax.plot(output[:num, i].squeeze().cpu().detach().numpy(), fx_dt[:num, i].squeeze().cpu().detach().numpy(),
                alpha=0.7, label='$NDCN \enspace of \enspace dx{%d}/dt$' % i)
        ax.arrow(output[50, i].squeeze().detach().numpy(), fx_dt[50, i].squeeze().cpu().detach().numpy(),
                 output[51, i].squeeze().detach().numpy() - output[50, i].squeeze().detach().numpy(),
                 fx_dt[51, i].squeeze().cpu().detach().numpy() - fx_dt[50, i].squeeze().cpu().detach().numpy(),
...
def dx_dt_true_figure(true_y, L, t, T, time_tick):
    ...
    # print(dx_dt_true)
    for i in range(0, 100, 10):
        ax.plot(true_y[i, :].cpu().detach().numpy(), dx_dt_true[i, :], alpha=0.7, label='$True \enspace of \enspace dx{%d}/df$' % i)
        # 加箭头
